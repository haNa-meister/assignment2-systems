...
def weighted_sum_backward(
    ...
    # 1. Gradient Output (Scalar per row)
    ...
    # 5. Partial Gradient Weight (Output)
    # Note: shape is (n_row_tiles, D) because each block writes its own result
    ...
    # -----------------------------------------------------------
    # Main Loop over D (Columns)
    # -----------------------------------------------------------
    for i in range(tl.cdiv(D, D_TILE_SIZE)):
        # Load Gradient of Output (dy)
        ...
        # --- Computation 1: Gradient w.r.t X (dx = dy * w) ---
        # Broadcast grad_output to (ROWS, 1) and weight to (1, D)
        grad_x_row = grad_output[:, None] * weight[None, :]
        # Store result
        tl.store(grad_x_block_ptr, grad_x_row, boundary_check=(0, 1))

        # --- Computation 2: Gradient w.r.t Weight (dw = sum(dy * x)) ---
        # We need to load X for this part
        x = tl.load(x_block_ptr, boundary_check=(0, 1), padding_option="zero")
        
        # Calculate contribution of this block to the weight gradient
        # grad_output: (ROWS, 1), x: (ROWS, D) -> result: (ROWS, D)
        # Then we sum over the rows (axis 0) to get (D,)
        grad_weight_chunk = tl.sum(grad_output[:, None] * x, axis=0)
        
        # Reshape to (1, D) to match the block pointer shape
        grad_weight_chunk = grad_weight_chunk[None, :]
        
        # Store partial result
        tl.store(partial_grad_weight_block_ptr, grad_weight_chunk, boundary_check=(1,))

        # -----------------------------------------------------------
        # Advance Pointers
        # -----------------------------------------------------------
        # Move D-dimension pointers forward by D_TILE_SIZE
        x_block_ptr = x_block_ptr.advance((0, D_TILE_SIZE))
        weight_block_ptr = weight_block_ptr.advance((D_TILE_SIZE,))
        grad_x_block_ptr = grad_x_block_ptr.advance((0, D_TILE_SIZE))
        partial_grad_weight_block_ptr = partial_grad_weight_block_ptr.advance((0, D_TILE_SIZE))
        
        # Note: grad_output_block_ptr does NOT advance because it is reused 
        # for every chunk of columns (it only depends on rows).
...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        grad_weight = partial_grad_weight.sum(axis=0) # Reduce over the partial results
